...
They can be provided either as environment variables or as Docker secrets.

Docker secrets take precedence when present and non-empty.
...
This allows you to start with tunnel routes only, then enable DNS or Access synchronization later.
...
Use dry-run mode for:

- first deployment;
- validating labels;
- production audits;
- checking cleanup behavior before enabling deletion.
...
This is useful for CI, manual sync jobs, or debugging.

## Poll interval

The controller periodically reconciles Docker labels with Cloudflare resources.

```bash
-e SYNC_POLL_INTERVAL=30s
```

A lower interval reacts faster but calls Cloudflare and Docker more often. A higher interval reduces activity but delays reconciliation.
...
`SYNC_DNS_ZONES` is useful when a whole zone disappears from current labels but you still want old managed DNS records in that zone to be cleaned up.

## Managed-by marker

`SYNC_MANAGED_BY` controls the marker used to identify resources managed by this controller.

```bash
-e SYNC_MANAGED_BY=docker-cf-tunnel-sync
```
...
Changing this value can affect how existing managed resources are detected.
